...
export default function AreaDetailPage() {
  ...
  useEffect(() => {
    ...
    async function loadArea() {
      try {
        ...
        if (!cancelled) {
          setError("Unable to load this area.");
          setArea(null);
        }
      } finally {
        if (!cancelled) {
  ...
  if (loading) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#EAE0CC] text-[#1A1712]">
        <div className="text-center">
          <div className="mx-auto mb-6 h-10 w-10 animate-spin rounded-full border border-[#1A1712]/15 border-t-[#B38B2E]" />

          <p className="text-xs uppercase tracking-[0.3em] text-[#756852]">
            Loading area
          </p>
        </div>
      </main>
  ...
  if (error || !area) {
    return (
      <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#EAE0CC] px-6 text-[#1A1712]">
        <div
          className="pointer-events-none absolute -left-40 top-0 h-[500px] w-[500px] rounded-full opacity-40 blur-[140px]"
          style={{
            background:
        ...
        <div className="relative z-10 max-w-xl text-center">
          <div className="mx-auto mb-8 flex h-16 w-16 items-center justify-center rounded-full border border-[#1A1712]/15">
            <Sparkles size={22} className="text-[#B38B2E]" />
          </div>

          <p className="mb-4 text-xs uppercase tracking-[0.3em] text-[#7A5C16]">
            Area unavailable
          </p>

          <h1 className="mb-8 text-4xl tracking-[-0.04em] md:text-6xl">
            {error || "Area not found."}
          </h1>

          <Link
            href="/#areas"
            className="group inline-flex items-center gap-3 border border-[#1A1712]/20 px-6 py-3 text-xs uppercase tracking-[0.15em] transition-all duration-300 hover:border-[#B38B2E] hover:bg-[#B38B2E] hover:text-white"
          >
            <ArrowLeft
              size={16}
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 mx-auto grid min-h-[calc(100vh-100px)] max-w-[1500px] items-center gap-10 py-16 lg:grid-cols-12 lg:gap-16 lg:py-20">
          ...
          <div
            ref={heroContentRef}
            className="relative lg:col-span-5 lg:pl-4"
          >
            <p
              ref={eyebrowRef}
